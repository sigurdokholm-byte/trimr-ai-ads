...
struct OBShampooQuiz: View {
    let onNext: () -> Void
    let onBack: () -> Void
    let progress: Double
    @Binding var value: ShampooFrequency?

    private struct Option { let freq: ShampooFrequency; let title: String }
    private let options: [Option] = [
        .init(freq: .everyday,     title: "Everyday"),
        .init(freq: .everyOtherDay, title: "Every other day"),
        .init(freq: .onceAWeek,    title: "Once a week"),
        .init(freq: .never,        title: "Never"),
    ]
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 28) {
                ...
                VStack(spacing: 12) {
                    ForEach(options, id: \.freq) { opt in
                        let selected = value == opt.freq
                        Button { value = opt.freq } label: {
                            Text(opt.title)
