...
/// Onboarding paywall step — the app's own custom StoreKit paywall (NOT
/// RevenueCat's `PaywallView`). RevenueCat stays initialised in observer mode
/// purely for revenue/customer tracking; it does not render or drive this UI.
/// Purchases run through the app-wide `StoreKitManager` (`app.store`) →
/// `validate-apple-iap` → `profiles.look_credits`.
struct OBPaywall: View {
    ...
    private func packCard(pack: Pack) -> some View {
        ...
        return VStack(spacing: 0) {
            // Badge slot keeps cards aligned even when no badge.
            ZStack {
